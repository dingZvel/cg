...
// Light info.
const GLfloat initial_lightPosition[] = {
    5.0f, 3.0f, 10.0f, 0.0f}; // Directional light. Assumed in eye space.
...
const GLfloat lightPosIncr =
    0.5f; // Step size to move light position in x & y directions.
...
static void SetupEnvMap(void) {
  ...
  const char *cubemapFile[numImages] = {
      "images/cm2_right.png",  "images/cm2_left.png", "images/cm2_top.png",
      "images/cm2_bottom.png", "images/cm2_back.png", "images/cm2_front.png"};
  ...
  for (int t = 0; t < numImages; t++) {
    ...
    GLubyte *imgData =
        stbi_load(cubemapFile[t], &imgWidth, &imgHeight, &numComponents, 0);

    if (imgData == NULL) {
      fprintf(stderr, "Error: Fail to read image file %s.\n", cubemapFile[t]);
      exit(EXIT_FAILURE);
    }
    printf("%s (%d x %d, %d components)\n", cubemapFile[t], imgWidth, imgHeight,
           numComponents);

    if (numComponents == 1) {
      glTexImage2D(target[t], 0, GL_R8, imgWidth, imgHeight, 0, GL_RED,
                   GL_UNSIGNED_BYTE, imgData);
    } else if (numComponents == 3) {
      glTexImage2D(target[t], 0, GL_RGB8, imgWidth, imgHeight, 0, GL_RGB,
                   GL_UNSIGNED_BYTE, imgData);
    } else {
...
static void SetupTextureMaps(void) {
  ...
  const char *texMapFile[numTexMaps] = {"images/brick_diffuse_map.png",
  ...
  for (int t = 0; t < numTexMaps; t++) {
    ...
    GLubyte *imgData =
        stbi_load(texMapFile[t], &imgWidth, &imgHeight, &numComponents, 0);

    if (imgData == NULL) {
      fprintf(stderr, "Error: Fail to read image file %s.\n", texMapFile[t]);
      exit(EXIT_FAILURE);
    }
    printf("%s (%d x %d, %d components)\n", texMapFile[t], imgWidth, imgHeight,
           numComponents);
    ...
    if (numComponents == 1) {
      glTexImage2D(GL_TEXTURE_2D, 0, GL_R8, imgWidth, imgHeight, 0, GL_RED,
                   GL_UNSIGNED_BYTE, imgData);
    } else if (numComponents == 3) {
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, imgWidth, imgHeight, 0, GL_RGB,
                   GL_UNSIGNED_BYTE, imgData);
    } else {
      fprintf(stderr, "Error: Unexpected image format.\n");
      exit(EXIT_FAILURE);
...
static void MyDrawFunc(void) {
  ...
  glm::mat4 projMat = glm::perspective(
      glm::radians(60.0f), (float)winWidth / winHeight, 0.1f, 100.0f);

  // View transformation matrix.
  glm::mat4 viewMat =
      glm::lookAt(glm::vec3(cam_eye[0], cam_eye[1], cam_eye[2]),
  ...
  //=======================================================================
  {
    ...
    glm::vec3 camWorldPos = glm::transpose(glm::mat3(camRotMat)) *
                            glm::vec3(cam_eye[0], cam_eye[1], cam_eye[2]);
    ...
    // Make the skybox translate with the camera's translation perfectly
    modelMat0 = glm::translate(modelMat0, camWorldPos);

    // Scale to skyboxSize
    modelMat0 =
        glm::scale(modelMat0, glm::vec3(skyboxSize, skyboxSize, skyboxSize));

    glm::mat4 modelViewMat0 = viewMat * modelMat0;
    glm::mat4 modelViewProjMat0 = projMat * modelViewMat0;
    glm::mat3 normalMat0 =
        glm::transpose(glm::inverse(glm::mat3(modelViewMat0)));
    ...
    shaderProg.setUniform(
        "WhichObj",
        0); // 0 -- draw skybox, 1 -- draw brick cube, 2 -- draw wooden cube.
  ...
  //=======================================================================
  {
    ...
    modelMat1 =
        glm::rotate(modelMat1, glm::radians(0.0f), glm::vec3(1.0f, 0.0f, 0.0f));

    glm::mat4 modelViewMat1 = viewMat * modelMat1;
    glm::mat4 modelViewProjMat1 = projMat * modelViewMat1;
    glm::mat3 normalMat1 =
        glm::transpose(glm::inverse(glm::mat3(modelViewMat1)));
  ...
  //=======================================================================
  // Set up transformations and other uniform variables and draw the wooden
  // cube.
  //=======================================================================
  {
    ...
    // Rotate 0 degrees about x-axis.
    modelMat2 =
        glm::rotate(modelMat2, glm::radians(0.0f), glm::vec3(1.0f, 0.0f, 0.0f));

    glm::mat4 modelViewMat2 = viewMat * modelMat2;
    glm::mat4 modelViewProjMat2 = projMat * modelViewMat2;
    glm::mat3 normalMat2 =
        glm::transpose(glm::inverse(glm::mat3(modelViewMat2)));
    ...
    shaderProg.setUniform(
        "WhichObj",
        2); // 0 -- draw skybox, 1 -- draw brick cube, 2 -- draw wooden cube.
...
/////////////////////////////////////////////////////////////////////////////
static void MyKeyboardFunc(GLFWwindow *window, int key, int scancode,
                           int action, int mods) {
...
/////////////////////////////////////////////////////////////////////////////
static void MyMouseClickFunc(GLFWwindow *window, int button, int action,
                             int mods) {
...
/////////////////////////////////////////////////////////////////////////////
static void MyMouseMotionFunc(GLFWwindow *window, double mouse_x,
                              double mouse_y) {
  float rotScale = 1.0f;
  float transScale = 2.0f;

  if (mouseLeftPressed) {
    trackball(cam_prev_quat,
              rotScale * (2.0f * prevMouseX - winWidth) / (float)winWidth,
...
int main(int argc, char **argv) {
  ...
  GLFWwindow *window =
      glfwCreateWindow(winWidth, winHeight, "main", NULL, NULL);
