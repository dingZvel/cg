...
void InitScene()
{
    ...
    Sphere[0].center = vec3(2.0 * sin(iTime) * sin(0.5 * iTime), 2, 0.0);
    Sphere[0].radius = 0.7;
    Sphere[0].materialID = 1;

    // Circling sphere.
    Sphere[1].center = vec3(3.0 * cos(-iTime) * sin(0.5 * iTime), 0.5, 1.5 * sin(-iTime));
    Sphere[1].radius = 0.5;
    ...
    // Gold material.
    Material[1].k_d = vec3(0.2, 0.4, 0.4);
    ...
    Material[2].k_d = vec3(0.3, 0.1, 0.2);
    ...
    // Light 0.
    Light[0].position = vec3(6.0 * cos(iTime), 8.0, 3.0 * sin(iTime));
    ...
    out float t, out vec3 hitPos, out vec3 hitNormal)
{
    ...
    Ray_t transRay = Ray_t(ray.o - sph.center, ray.d);
    float a = dot(transRay.d, transRay.d);
    float b = 2.0 * dot(transRay.d, transRay.o);
    float c = dot(transRay.o, transRay.o) - sph.radius * sph.radius;
    float d = b * b - 4.0 * a * c;
    ...
    if (t_plus < 0.0 || t_plus < tmin || t_minus > tmax || (t_plus > tmax && t_minus < tmin))
        return false;

    t = t_minus >= max(tmin, 0.0) ? t_minus : t_plus;
    ...
    out vec3 hitNormal, out vec3 k_rg)
{
    ...
    // Check for intersections with all planes
    ...
    // Loop through all lights to accumulate illumination
    for (int i = 0; i < NUM_LIGHTS; i++) {
        vec3 lightVector = Light[i].position - nearest_hitPos;
        float distToLight = length(lightVector);
        vec3 L = normalize(lightVector);

        Ray_t shadowRay;
        shadowRay.o = nearest_hitPos; // Start at the intersection point
        shadowRay.d = L; // Point towards the light

        bool inShadow = false;

        for (int j = 0; j < NUM_PLANES; j++) {
            if (IntersectPlane(Plane[j], shadowRay, DEFAULT_TMIN, distToLight)) {
                inShadow = true;
                break;
            }
        }
        ...
        if (!inShadow) {
            for (int j = 0; j < NUM_SPHERES; j++) {
                if (IntersectSphere(Sphere[j], shadowRay, DEFAULT_TMIN, distToLight)) {
                    inShadow = true;
                    break;
                }
            }
        ...
        I_local += PhongLighting(L, nearest_hitNormal, V, inShadow, Material[nearest_hitMatID], Light[i]);
    }
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    for (int s = 0; s < SPP; s++) {
        ...
        // Anti-Aliasing (Jittering the pixel coordinate)
        vec2 jitteredCoord = fragCoord + vec2(rand() - 0.5, rand() - 0.5);

        // Calculate the target position on the image plane (plane of focus) in camera space
        vec3 pixel_pos = image_origin + vec3(pixel_width * jitteredCoord, 0.0);

        // Convert the target focal point to world space
        vec3 focal_point = cam_pos +
                pixel_pos.x * cam_x_axis +
                pixel_pos.y * cam_y_axis +
                pixel_pos.z * cam_z_axis;

        // Depth of Field (Jittering the ray origin on the lens)
        vec2 lens_uv = (vec2(rand(), rand()) - 0.5) * aperture_width;

        // Create the primary ray for this specific sample
        Ray_t pRay;
        // The ray originates from the randomly sampled point on the lens
        pRay.o = cam_pos + lens_uv.x * cam_x_axis + lens_uv.y * cam_y_axis;
        // The ray direction goes from the new lens origin to the focal point
        pRay.d = normalize(focal_point - pRay.o);
        ...
        final_color += I_result;
    }

    // Average the accumulated color across all samples to get the final pixel color
    fragColor = vec4(final_color / float(SPP), 1.0);
}
