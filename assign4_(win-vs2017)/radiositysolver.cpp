...
// Note that numPixelsOnWidth must be a even number.
{
    ...
    **********************************************************/
    ...
    int height = numPixelsOnWidth / 2;     // Height of the side face.
    ...
    for (int py = 0; py < height; py++)
    {
        ...
        for (int px = 0; px < numPixelsOnWidth; px++)
        {
            ...
            // Distance from the pixel to the center of the side face.
            double dFq = dA * y / (M_PI * Sqr(x * x + y * y + 1.0));
            deltaFormFactors[py * numPixelsOnWidth + px] = (float)dFq;
        }
    }
}
...
// Need to set up the viewport, projection and view transfromation.
{
    ...
    **********************************************************/
    ...
    float lookDir[3], upVector[3], lookAt[3];
    CopyArray3(upVector, shooterQuad->normal);

    float v01[3], u[3];
    VecDiff(v01, shooterQuad->v[1], shooterQuad->v[0]);
    VecCrossProd(u, v01, upVector);

    if (face == 1) {
        CopyArray3(lookDir, v01);
    }
    else if (face == 2) {
        CopyArray3(lookDir, u);
    }
    else if (face == 3) {
        VecScale(lookDir, -1.0f, v01);
    }
    else if (face == 4) {
        VecScale(lookDir, -1.0f, u);
    }
    ...
    // and update the unshot power of their parent shooter quads.
{
    for (int i = 0; i < width * height; i++)
    {
        ...
        **********************************************************/
        ...
        for (int j = 0; j < 3; j++)
        {
            // The power received by the gatherer quad for each color channel.
            deltaPower[j] = gathererQuad->surface->reflectivity[j] * shotPower[j] * dF;
            // Update the radiosity of the gatherer quad for each color channel.
            gathererQuad->radiosity[j] += deltaPower[j] / gathererQuad->area;
            // Update the unshot power of the parent shooter quad for each color channel.
            gathererQuad->shooter->unshotPower[j] += deltaPower[j];
		}
    }
}
