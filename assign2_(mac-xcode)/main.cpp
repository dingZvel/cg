...
glm::mat4 shadowMatrix; // Transforms point in world coordinates to shadowmap
                        // coordinates.
...
static void SetUpShadowMapAndFBO(void) {
  ...
  // Allocate texture memory for depth component
  ...
  // Clamp to border so coordinates outside [0, 1] return the border color
...
static void ComputeLightPosition(void) {
  ...
  xformMat = glm::rotate(xformMat, glm::radians(lightLongitude),
                         glm::vec3(0.0f, 1.0f, 0.0f));
  xformMat = glm::rotate(xformMat, glm::radians(lightLatitude),
...
static void RenderObjects(const glm::mat4 &viewMat, const glm::mat4 &projMat) {
  { // Draw the floor (-y) plane.
    ...
    modelMat = glm::rotate(modelMat, glm::radians(-15.0f),
                           glm::vec3(0.0f, 1.0f, 0.0f));
  ...
  { // Draw the right (+x) plane.
    ...
    modelMat =
        glm::rotate(modelMat, glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f));
    modelMat = glm::scale(modelMat, glm::vec3(8.0f, 8.0f, 8.0f));
  ...
  { // Draw the back (-z) plane.
    ...
    modelMat =
        glm::rotate(modelMat, glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
    modelMat = glm::scale(modelMat, glm::vec3(8.0f, 8.0f, 8.0f));
  ...
  { // Draw the box.
    ...
    modelMat = glm::rotate(modelMat, glm::radians(-45.0f),
                           glm::vec3(0.0f, 1.0f, 0.0f));
  ...
  { // Draw the teapot.
    ...
    modelMat = glm::rotate(modelMat, glm::radians(150.0f),
                           glm::vec3(0.0f, 1.0f, 0.0f));
    modelMat = glm::rotate(modelMat, glm::radians(-90.0f),
...
static void RenderShadowMap(void) {
  ...
  glm::mat4 projMat = glm::perspective(
      glm::radians(lightVerticalFOV), (float)fboWidth / fboHeight, 2.0f, 30.0f);
  ...
  glm::mat4 shadowMatrix = biasMatrix * projMat * viewMat;
...
static void MyDrawFunc(void) {
  ...
  // Perspective projection matrix.
  glm::mat4 projMat = glm::perspective(
      glm::radians(60.0f), (float)winWidth / winHeight, 0.5f, 100.0f);
  ...
  glm::vec3 ecLightPosition =
      glm::vec3(viewMat * glm::vec4(lightPosition, 1.0f));
...
static void MyKeyboardFunc(GLFWwindow *window, int key, int scancode,
  ...
  if (action == GLFW_PRESS || action == GLFW_REPEAT) {
    ...
    // Move the light.
      ...
      if (lightLongitude < -360.0f)
        lightLongitude += 360.0f;
      ComputeLightPosition();
